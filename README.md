# Digital-literacy
# Name - Nilanjana
# registration number - 25BCE11181
# 📘 Digital Literacy Project – CSE0001

##  Project Overview
This repository contains my submission for the **Digital Literacy Project** under course code **CSE0001** at **VIT Bhopal University**.  
The project demonstrates my learning across five modules, covering:
- Digital awareness and safe internet practices  
- Building a professional online portfolio  
- Exploring coding and collaboration platforms  
- Practicing professional communication and etiquette  
- Understanding cybercrime and prevention strategies  
---

##  Repository Structure


README.md report/Project_Report.pdf task-1-presentation/ task-2-portfolio/ task-3-platforms/ task-4-email-etiquette/ task-5-cybercrime/

---

## 📑 Module Summaries

### **Task 1 – Digital Literacy Awareness Infographic**
- Tool used: Canva/Prezi/Miro  
- Deliverable: Infographic explaining digital literacy, safe internet practices, and professional presence.  
- Reflection: Written in report (150–200 words).  
- 📎 [Infographic Screenshot/Link]

---

### **Task 2 – Student Digital Portfolio**
- Platforms created: GitHub, LinkedIn, Kaggle/Stack Overflow/ResearchGate  
- Deliverable: Screenshots of profiles + reflection notes.  
- Reflection: Written in report (150–200 words).  
- 📎 [Profile Screenshots]

---

### **Task 3 – Coding & Collaboration Platforms**
- Coding Practice: Beginner challenge completed on HackerRank/CodeChef/HackerEarth.  
- Collaboration: Google Form quiz (5 questions) + linked Google Sheet responses.  
- Reflection: Written in report (150–200 words).  
- 📎 [Form Link + Screenshots]

---

### **Task 4 – Email Etiquette & Social Media Guide**
- Two professional email drafts:
  1. Requesting assignment extension (to professor).  
  2. Internship interest email (to coordinator).  
- Social Media Do’s & Don’ts checklist (5 each).  
- Reflection: Written in report (100–150 words).  
- 📎 [Files Here]

---

### **Task 5 – Cybercrime Awareness Case Study**
- Case Study: [Chosen topic – e.g., Phishing/UPI Fraud/Identity Theft]  
- Prevention Checklist: 8 actionable tips (including UPI safety).  
- Reporting Channels: cybercrime.gov.in | Helpline 1930  
- Reflection: Written in report (100–150 words).  
- 📎 [Files Here]

---

##  References
- Canva – [https://www.canva.com](https://www.canva.com)  
- GitHub – [https://github.com](https://github.com)  
- LinkedIn – [https://www.linkedin.com](https://www.linkedin.com)  
- HackerRank – [https://www.hackerrank.com](https://www.hackerrank.com)  
- National Cyber Crime Portal – [https://cybercrime.gov.in](https://cybercrime.gov.in)  

---

##  Notes
- All reflections are written in my own words.  
- Screenshots are from my personal accounts.  
- This repository is part of my permanent digital portfolio.  



This README is complete, professional, and matches the assignment brief. You just need to insert your name, reg no., branch, screenshots, and links.
👉 Would you like me to also prepare a ready-to-use Project Report template (with headings, word limits, and placeholders), so you can start writing directly without worrying about formatting?
